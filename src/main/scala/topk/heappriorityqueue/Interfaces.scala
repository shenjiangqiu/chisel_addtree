...
/** contains relevant bundle types and port types for the heap-based priority
  * queue
  */
object Interfaces {
  ...
  class Event(implicit parameters: PriorityQueueParameters) extends Bundle {
    ...
    def <(that: Event): Bool =
      superCycle < that.superCycle || (superCycle === that.superCycle && cycle < that.cycle)
    def >(that: Event): Bool =
      superCycle > that.superCycle || (superCycle === that.superCycle) && cycle > that.cycle
    def ===(that: Event): Bool =
      superCycle === that.superCycle && cycle === that.cycle
  }

  class TaggedEvent(implicit parameters: PriorityQueueParameters)
      extends Bundle {
    import parameters._
    val event = new Event
  ...
  class searchPort(implicit parameters: PriorityQueueParameters)
      extends Bundle { // as seen from requester side
    import parameters._
