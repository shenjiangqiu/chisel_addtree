package topk.heappriorityqueue

import chisel3._
import chisel3.util._

import topk.heappriorityqueue.Interfaces._
import topk.heappriorityqueue.modules.{QueueControl, linearSearchMem}

case class PriorityQueueParameters(
    size: Int,
    order: Int,
    superCycleWidth: Int,
    cycleWidth: Int,
    referenceIdWidth: Int
)

/** Component implementing a priority queue, where the minimum value gets to the
  * head of the queue
  ...
  *     - a user generated reference ID given at insertion which is used to
  ...
  *     - a component which is able to search through the reference ID fields
  *       and return the index where a given reference ID is present
  *       - the queue waits until the search result is given
  *   - the head element is kept locally in a FF for symmetry and access speed
  *     reasons
  ...
  * @param order
  *   the number of children per node in the tree. Must be power of 2
  * @param nWid
  *   width of the normal priority value
  * @param cWid
  *   width of the cyclic priority value
  * @param rWid
  *   width of the reference ID tags
  */
class PriorityQueue(
    size: Int,
    order: Int,
    superCycleRes: Int,
    cyclesPerSuperCycle: Int,
    exposeState: Boolean = false
) extends Module {
  require(
    isPow2(order),
    "The number of children per node needs to be a power of 2!"
  )

  val superCycleWidth = superCycleRes
  val cycleWidth = log2Ceil(cyclesPerSuperCycle)
  val referenceIdWidth = log2Ceil(size + 1)
  implicit val parameters = PriorityQueueParameters(
    size,
    order,
    superCycleWidth,
    cycleWidth,
