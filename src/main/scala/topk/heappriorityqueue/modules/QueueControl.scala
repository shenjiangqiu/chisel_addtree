...
/** Component implementing the control for the priority queue
  */
class QueueControl(implicit parameters: PriorityQueueParameters)
    extends Module {
  import parameters._
  require(
    isPow2(order),
  ...
  val headReg = RegInit(
    VecInit(Seq.fill(cycleWidth + superCycleWidth + referenceIdWidth)(1.U))
      .asTypeOf(new TaggedEvent)
  )
  ...
  val wrIndexOffset = Mux(
    wrIndex === 0.U,
    0.U,
    wrIndex(log2Ceil(order), 0) - 1.U(log2Ceil(size).W)
  )

  // ram read address and offset
  val rdIndex = WireDefault(0.U(log2Ceil(size).W))
  val rdIndexToRam = ((rdIndex - 1.U) >> log2Ceil(order)).asUInt
  val rdIndexOffset = Mux(
    rdIndex === 0.U,
    0.U,
    rdIndex(log2Ceil(order), 0) - 1.U(log2Ceil(size).W)
  )

  val resetVal = VecInit(
    Seq.fill(cycleWidth + superCycleWidth + referenceIdWidth)(true.B)
  ...
  switch(stateReg) {
    ...
    is(normalInsertion) { // insertion into already filled queue
      ...
      heapifier.io.control.idx := Mux(
        heapSizeReg < (order + 1).U,
        0.U,
        ((heapSizeReg - 1.U) >> log2Ceil(order)).asUInt
      )
    ...
    is(removal) {
      ...
      heapifier.io.control.heapifyUp := true.B
      heapifier.io.control.idx := Mux(
        removalIndex < (order + 1).U,
        0.U,
        ((removalIndex - 1.U) >> log2Ceil(order)).asUInt
      )
      stateReg := waitForHeapifyUp
    }
