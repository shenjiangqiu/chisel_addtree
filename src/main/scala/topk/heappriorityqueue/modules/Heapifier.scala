package topk.heappriorityqueue.modules

import chisel3._
import chisel3.util._

import topk.heappriorityqueue.Interfaces.{TaggedEvent, rdPort, wrPort}
import topk.heappriorityqueue.PriorityQueueParameters

/** Component implementing the algorithm to create a min-heap. Works itself
  * either upwards or downwards through the heap in memory from a given start
  * index and swaps elements to satisfy the min-heap condition. The start index
  * is always the parent index.
  *
  * During operation either of the control signals heapifyUp or heapifyDown need
  * to be held high until done is asserted.
  *
  * When done is asserted, the component also signalizes whether a swap has
  * taken place
  */
class Heapifier(implicit parameters: PriorityQueueParameters) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val control = new Bundle {
      ...
      val heapSize =
        Input(UInt(log2Ceil(size + 1).W)) // the current size of the heap
    }

    // ram ports
    val rdPort = new rdPort(log2Ceil(size / order), Vec(order, new TaggedEvent))
    val wrPort =
      new wrPort(log2Ceil(size / order), order, Vec(order, new TaggedEvent))
  ...
  val minFinder = Module(
    new MinFinder(order + 1)
  ) // module to find the minimum priority among the parent and children

  // state elements
  val idle :: preUp1 :: preDown1 :: preUp2 :: preDown2 :: readUp :: readDown :: wbUp1 :: wbDown1 :: wbUp2 :: wbDown2 :: Nil =
    Enum(11)
  val stateReg = RegInit(idle) // state register
  val indexReg = RegInit(
    0.U(log2Ceil(size).W)
  ) // register holding the index of the current parent
  val swappedReg = RegInit(
    false.B
  ) // register holding a flag showing whether a swap has occurred
  val parentReg = RegInit(
    VecInit(Seq.fill(order)(0.U.asTypeOf(new TaggedEvent)))
  ...
  val ramAddressChildren =
    addressIndex // the RAM addres of the children equals the index of the parent
  val ramAddressParent =
    ((addressIndex - 1.U) >> log2Ceil(order))

  // parent selection
  val parentOffset = Mux(
    indexReg === 0.U,
    0.U,
    indexReg(log2Ceil(order), 0) - 1.U(log2Ceil(size).W)
  ) // the offset of the parent within its RAM cell
  val parent = parentReg(
    parentOffset
  ) // the actual parent selected from the parent register
  ...
  val nextIndexUp =
    ((indexReg - 1.U) >> log2Ceil(order))
  val nextIndexDown = (indexReg << log2Ceil(order)) + RegNext(
    minFinder.io.index
  ) // index of next parent is given by (index * childrenCount) + selected child
  val swapRequired =
    minFinder.io.index =/= 0.U // a swap is only required when the parent does not have the highest priority
